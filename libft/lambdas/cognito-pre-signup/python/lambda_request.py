...
class Request:
    ...
    def _is_not_ajuma_email_user(self, email):
        domain_name = email.split('@')[1]
        return domain_name != 'ajuma.com'
    ...
    def __call__(self, event, context):
        ...
        if self._is_not_prod() and self._is_not_ajuma_email_user(payload):
            reason = "invalid request json - domain name is not 'ajuma.com'"
            logger.error(reason)
            raise Exception(
