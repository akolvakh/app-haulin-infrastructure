...
masking_filter = filter.MaskingFilter(['phone_number', 'birthdate', 'cognito:email_alias'])
    ...
    def test_log_masking(self):
        input_json = {
            "userAttributes": {
                ...
                "userName": "user123",
                "cognito:email_alias": "test1@gmail.com"
            }
        }

        with self.assertLogs() as log_content:
            logger.warning(input_json)
            record = log_content.records[0]
            masking_filter.filter(record)
            self.assertIn('\'cognito:email_alias\': \'***************\'', str(record))
