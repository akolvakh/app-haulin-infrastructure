...
resource "aws_codepipeline" "default" {
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ...
        DetectChanges        = ("dev" == local.common.Environment) ? true : false
      }
    }
    action {
      name             = "TF-code-source"
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ...
        ProjectName          = "${var.codepipeline_label}-tf-appsync-resolvers-list-build"
        EnvironmentVariables = <<-EOT
          [
            {"name":"TF_CODE_BRANCH","value":"${var.tf_code_branch}","type":"PLAINTEXT"},
            {"name":"TERRAFORM_MODULE_PATH","value":"${var.appsync_resolvers_list_module_path}","type":"PLAINTEXT"},
  ...
  stage {
    ...
    action {
      name     = "TF_Plan_AppSync_Resolvers"
      category = "Build"
      owner    = "AWS"
      provider = "CodeBuild"
      version  = "1"
      ...
      input_artifacts  = ["checked_out_terraform_code", "checked_out_code"]
      output_artifacts = ["tfplan_appsync_resolvers"]
      ...
      configuration = {
        ...
        EnvironmentVariables = <<-EOT
          [
            {"name":"TF_CODE_BRANCH","value":"${var.tf_code_branch}","type":"PLAINTEXT"},
            {"name":"TERRAFORM_MODULE_PATH","value":"${var.appsync_deploy_module_path}","type":"PLAINTEXT"},
            {"name":"RESOLVERS_DIR","value":"${var.appsync_resolvers_dir}","type":"PLAINTEXT"},
            {"name":"ECS_ENV","value":"${var.environment}","type":"PLAINTEXT"}
          ]
        EOT
      }
  ...
  stage {
    ...
    action {
      name     = "TF_Apply_AppSync_Resolvers"
      category = "Build"
      owner    = "AWS"
      provider = "CodeBuild"
      version  = "1"

      input_artifacts = ["tfplan_appsync_resolvers", "checked_out_terraform_code", "checked_out_code"]

      configuration = {
        PrimarySource        = "tfplan_appsync_resolvers"
        ProjectName          = "${var.codepipeline_label}-tf-apply-build"
        EnvironmentVariables = <<-EOT
          [
            {"name":"TF_CODE_BRANCH","value":"${var.tf_code_branch}","type":"PLAINTEXT"},
