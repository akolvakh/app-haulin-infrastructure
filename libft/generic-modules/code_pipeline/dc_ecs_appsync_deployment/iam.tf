...
resource "aws_iam_role" "tf_command_codebuild_role" {
  name               = "${var.codepipeline_label}-codebuild-tf-command"
...
resource "aws_iam_policy" "cloud_watch_for_tf_command_build" {
  ...
  tags = merge(
    var.tags,
    {
      Description = "IAM policy for ${join("", aws_iam_role.tf_command_codebuild_role.*.id)} to allow cloudwatch API calling"
    },
    {
      Parent = "CodePipeline label: ${var.codepipeline_label}"
    }
...
resource "aws_iam_policy" "ecs_service_deploy" {
  ...
  tags = merge(
    var.tags,
    {
      Description = "IAM policy for ${join("", aws_iam_role.codebuild_role.*.id)} to allow AWS ECS service deployment"
    },
    {
      Parent = "CodePipeline label: ${var.codepipeline_label}"
    }
...
resource "aws_iam_policy" "appsync_access" {
  name   = "${var.codepipeline_label}_build_appsync_access"
...
resource "aws_iam_role_policy_attachment" "appsync_access" {
  role       = join("", aws_iam_role.tf_command_codebuild_role.*.id)
  policy_arn = join("", aws_iam_policy.appsync_access.*.arn)
}
