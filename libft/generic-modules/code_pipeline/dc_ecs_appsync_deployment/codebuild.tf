...
resource "aws_codebuild_project" "tf-appsync-resolvers-list-project" {
  name          = "${var.codepipeline_label}-tf-appsync-resolvers-list-build"
  build_timeout = "5"
  service_role  = aws_iam_role.tf_command_codebuild_role.arn
  ...
  source {
    type      = "CODEPIPELINE"
    buildspec = file("${path.module}/tpl/buildspec_tf_resolvers_list.yaml.tpl")
  }
