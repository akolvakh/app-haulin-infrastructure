...
resource "aws_lb_target_group" "api_tgt" {
  ...
  port  = 7000 // all APIs to use standard port
  ...
  health_check {
    ...
    matcher             = "200"
    path                = "/status" // can we have stadard healthcheck path not tied to a Spring Framework specific? just "/health"??
...
resource "aws_lb_target_group" "web_tgt" {
  ...
  health_check {
    ...
    interval            = "30"
    matcher             = "200"
    path                = "/status" //"/health" // what is healthcheck path our java apps/services use, should be standard one
