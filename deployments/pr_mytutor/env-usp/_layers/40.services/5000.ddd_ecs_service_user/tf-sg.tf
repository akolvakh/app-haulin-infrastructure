  ...
  ingress = [
    {
      ...
      prefix_list_ids  = []
      protocol         = "tcp"
      security_groups  = [var.outputs_sg_sg_alb_public_app_id, var.outputs_ecs_service_api_gateway_sg_id, var.outputs_sg_sg_ecs_import_id, var.outputs_sg_sg_ecs_program_management_id, var.outputs_sg_sg_ecs_payment_id]
    }
  ]
  egress = [
    {
...
resource "aws_security_group_rule" "quartz" {
  type                      = "ingress"
  from_port                 = 5432
  to_port                   = 5432
  protocol                  = "tcp"
  security_group_id         = var.outputs_sg_sg_quartz_db_id
  source_security_group_id  = module.sg.sg_id
}
