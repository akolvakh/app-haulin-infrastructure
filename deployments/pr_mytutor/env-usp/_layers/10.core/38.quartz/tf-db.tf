...
module "db_subnet_group" {
  ...
  db_subnet_group_name = "${module.label.id}-quartz"
...
resource "aws_rds_cluster_parameter_group" "rds_aurora" {
  name        = "${module.label.id}-parameter-group-quartz-v11"
...
module "rds_aurora" {
  ...
  create_cluster         = true
  app                    = "${module.label.tags["Product"]}-quartz"
  env                    = module.label.tags["Environment"]
  db_subnet_group_name   = module.db_subnet_group.db_subnet_group_name
  db_cluster_identifier  = "${module.label.id}-quartz"
  ...
  db_vpc_security_group_ids = [aws_security_group.sg.id]
