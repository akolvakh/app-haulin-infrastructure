...
variable "ecs_interview_logging_level_com_phoenix" {
  description = "CD-34"
  default = "DEBUG"
}
...
module "label" {
  ...
  name       = var.profile.product
  stage      = var.profile.environment
  delimiter  = "_"
  tags       = {
    "Product"       = var.profile.product,
    "Contact"       = var.profile.contact,
    "Environment"   = var.profile.environment,
    "Orchestration" = var.profile.tag_orchestration,
    "Region"        = var.profile.aws_region
    "Layer"         = "10.vpc",
    "Jira"          = "MYT-1"
  }
...
module_name    = var.profile.tag_orchestration
module_version = var.profile.tf_framework_component_version
...
label                                                                  = module.label.tags
tf_framework_component_version                                         = "${var.profile.tf_framework_component_version}_s3"
}
#--------------------------------------------------------------
################################################################
#--------------------------------------------------------------
...
label                                                                   = module.label.tags
tf_framework_component_version                                          = "${var.profile.tf_framework_component_version}_secrets"
}
#--------------------------------------------------------------
################################################################
#--------------------------------------------------------------
...
label                                                                   = module.label.tags
tf_framework_component_version                                          = "${var.profile.tf_framework_component_version}_dns"
}
#--------------------------------------------------------------
################################################################
#--------------------------------------------------------------
...
acm_cloudfront                                                          = local.locals_acm_cloudfront_name[var.profile.environment]
acm_loadbalancer                                                        = local.locals_acm_alb_name[var.profile.environment]
tf_framework_component_version                                          = "${var.profile.tf_framework_component_version}_acm"
}
...
label                                                                  = module.label.tags
tf_framework_component_version                                         = "${var.profile.tf_framework_component_version}_static_ips"
}
#--------------------------------------------------------------
################################################################
#--------------------------------------------------------------
...
tf_framework_component_version                                          = "${var.profile.tf_framework_component_version}_vpc"
}
output "vpc_id" {
...
label                                                                   = module.label.tags
vpc_cidr                                                                = var.profile.vpc_cidr
tf_framework_component_version                                          = "${var.profile.tf_framework_component_version}_sg"
#connections
...
label                                                                  = module.label.tags
vpc_cidr                                                               = var.profile.vpc_cidr
tf_framework_component_version                                         = "${var.profile.tf_framework_component_version}_cache"
#connections
...
label                                                                  = module.label.tags
tf_framework_component_version                                         = "${var.profile.tf_framework_component_version}_kafka"
#connections
...
label                                                                  = module.label.tags
vpc_cidr                                                               = var.profile.vpc_cidr
tf_framework_component_version                                         = "${var.profile.tf_framework_component_version}_db"
#connections
...
label                                                                  = module.label.tags
tf_framework_component_version                                         = "${var.profile.tf_framework_component_version}_bastion"
#connections
...
label                                                                  = module.label.tags
vpc_cidr                                                               = var.profile.vpc_cidr
tf_framework_component_version                                         = "${var.profile.tf_framework_component_version}_quartz"
#connections
outputs_vpc_vpc_cidr_block                                             = module.vpc.vpc_cidr_block
outputs_vpc_public_subnets                                             = module.vpc.public_subnets[0]
outputs_vpc_vpc_id                                                     = module.vpc.vpc_id
...
label                                                                   = module.label.tags
clever_url                                                              = var.profile.clever_url //"https://uss-prod.auth.us-east-1.amazoncognito.com/oauth2/authorize?response_type=code&client_id=23jsa9lus4uq8ik2rfm9pjg6pm&redirect_uri=https%3A%2F%2Fportal.mytutorlearning.co%2Fauth%2Flogin&state=qweqw2131a123a1231212&identity_provider=Clever&scope=aws.cognito.signin.user.admin+openid+profile"
tf_framework_component_version                                          = "${var.profile.tf_framework_component_version}_lambda_clever"
#connections
...
label                                                                  = module.label.tags
tf_framework_component_version                                         = "${var.profile.tf_framework_component_version}_lambda_cognito_post_authentication"
#connections
outputs_sg_sg_rds_lambda_id                                            = module.sg.sg_rds_lambda_id
outputs_vpc_private_subnets                                            = module.vpc.private_subnets
}
...
label                                                                  = module.label.tags
tf_framework_component_version                                         = "${var.profile.tf_framework_component_version}_lambda_cognito_pre_authentication"
#connections
outputs_sg_sg_rds_lambda_id                                            = module.sg.sg_rds_lambda_id
outputs_vpc_private_subnets                                            = module.vpc.private_subnets
}
...
tf_framework_component_version                                          = "${var.profile.tf_framework_component_version}_lambda_cognito_post_confirmation"
#connections
outputs_sg_sg_rds_lambda_id                                             = module.sg.sg_rds_lambda_id
outputs_vpc_private_subnets                                             = module.vpc.private_subnets
}
...
label                                                                  = module.label.tags
external_dns_domain                                                    = var.profile.external_dns_domain
ses_aws_region                                                         = var.profile.ses_aws_region
sns_monthly_spend_limit                                                = var.profile.sns_monthly_spend_limit
vpc_cidr                                                               = var.profile.vpc_cidr
cognito_urls                                                           = [var.profile.tutorcommunity_booknook_callback_url, "https://${module.label.tags["Product"]}-${module.label.tags["Environment"]}.auth.${module.label.tags["Region"]}.amazoncognito.com/login/cognito", "https://localhost:3000/auth/login" ] //"https://staging-red-staging.auth.us-east-1.amazoncognito.com/login/cognito" //var.cognito_urls // "https://uss-prod.auth.us-east-1.amazoncognito.com/login/cognito" //"https://portal.mytutorlearning.co/auth/login" //["https://${var.label["Product"]}.${var.label["Environment"]}.${var.external_dns_domain}/auth/login"]
cognito_mail_from_domain                                               = var.profile.cognito_email_from_domain
cognito_ses_domain                                                     = var.profile.cognito_ses_domain
cogntio_ses_sender                                                     = var.profile.cogntio_ses_sender
cogntio_ses_email_subject                                              = var.profile.cogntio_ses_email_subject
...
schemas                                                                = local.locals_cognito_schemas[var.profile.environment]
tf_framework_component_version                                         = "${var.profile.tf_framework_component_version}_cognito"
#connections
...
label                                                                  = module.label.tags
alias                                                                  = local.locals_cloudfront_alias[var.profile.environment]
tf_framework_component_version                                         = "${var.profile.tf_framework_component_version}_cloudfront_web"
#connections
outputs_vpc_vpc_nat_eips_cidr                                          = module.vpc.vpc_nat_eips_cidr
outputs_acm_cloudfront_arn                                             = local.locals_acm_cloudfront[var.profile.environment] != "" ? local.locals_acm_cloudfront[var.profile.environment] : module.acm.acm_cloudfront_arn
}
...
label                                                                  = module.label.tags
vpc_cidr                                                               = var.profile.vpc_cidr
tf_framework_component_version                                         = "${var.profile.tf_framework_component_version}_alb"
#connections
outputs_acm_loadbalancer_arn                                           = local.locals_acm_alb[var.profile.environment] != "" ? local.locals_acm_alb[var.profile.environment] : module.acm.acm_loadbalancer_arn
...
label                                                                  = module.label.tags
tf_framework_component_version                                         = "${var.profile.tf_framework_component_version}_service_discovery"
#connections
outputs_vpc_vpc_id                                                     = module.vpc.vpc_id
}
#--------------------------------------------------------------
...
label                                                                  = module.label.tags
vpc_cidr                                                               = var.profile.vpc_cidr
tf_framework_component_version                                         = "${var.profile.tf_framework_component_version}_ecs"
}
output "ecs_cluster" {
...
label                                                                  = module.label.tags
boooknook_base_app_url                                                 = var.profile.booknook_base_app_url
external_dns_domain                                                    = var.profile.external_dns_domain
allowed_origin_patterns                                                = var.profile.allowed_origin_patterns
...
platform_name                                                          = var.profile.platform_name
phoenix_master_db_schema                                               = var.profile.phoenix_master_db_schema
tf_framework_component_version                                         = "${var.profile.tf_framework_component_version}_ddd_ecs_service_api_gateway"
#connections
...
first_run                                                              = var.first_run
label                                                                  = module.label.tags
boooknook_base_app_url                                                 = var.profile.booknook_app_url
external_dns_domain                                                    = var.profile.external_dns_domain
csrf_enabled                                                           = var.profile.csrf_enabled
server_secret                                                          = var.profile.server_secret
server_base_url                                                        = var.profile.server_base_url
...
ecs_import_kafka_producer_tx_prefix                                    = var.profile.ecs_import_kafka_producer_tx_prefix
smoketest_emails                                                       = var.profile.smoketest_emails
spring_profiles_active                                                 = var.profile.spring_profiles_active
platform_name                                                          = var.profile.platform_name
phoenix_master_db_schema                                               = var.profile.phoenix_master_db_schema
tf_framework_component_version                                         = "${var.profile.tf_framework_component_version}_ddd_ecs_service_import"
#connections
#   outputs_alb_alb_public_app_backends_api_gateway                        = module.alb.alb_public_app_backends_api_gateway
outputs_alb_listener_arn                                               = module.alb.aws_lb_listener_https
outputs_cache_cache_entrypoint                                         = module.cache.cache_entrypoint
...
#label                           = module.label.tags
#tf_framework_component_version  = "${var.profile.tf_framework_component_version}_etc_openvpn_marketplace"
##connections
